...
def debug_print(fmt, *args, predicate=None, threadNumber=-1, uniform=True, forcePrint=False):
    ...
    with ir.InsertionPoint(module.body):
        ...
        def mlir_matmul_warpspecialized(a_host, b_host, c_host):
            ...
            with ir.InsertionPoint(launch_op.body.blocks[0]):
                ...
                memref.assume_alignment(c_device, 128)
                dynamic_smem = gpu.dynamic_shared_memory(
                    ir.MemRefType.get((MLIR_DYNAMIC, ), i8, memory_space=smem_space))
                ticks = c(10000000)

                # GPU Step 2. Find the primary thread, warps, warp groups and etc.
                tidx = gpu.thread_id(gpu.Dimension.x)
                ...
                # GPU Step 3. Initialize mbarrier groups
                ...
                # GPU Step 4. Prefetch TMA descriptors
                ...
                with ir.InsertionPoint(scf.IfOp(is_producer).then_block):
                    ...
                    with ir.InsertionPoint(for_op.body):
                        ...
                        nvvm.BarrierOp(barrierResource=1, numberOfThreads=arith.constant(i32, WARP_GROUP_SIZE))

                        # Step 5.2.1. Wait mbar2
                ...
                with ir.InsertionPoint(if_op.then_block):
                    ...
                    with ir.InsertionPoint(for_op.body):
                        ...
                        debug_print("[cons] {}  | mbarTMA[{}] try_wait   phase={} [done]",
                                    ...
                                    phaseParity,
                                    predicate=consumerPrimaryThread)
                        # Step 6.2.2. Create WGMMA Descriptors
                        a_offset = arith.muli(stage, c(lhs_tile_bytes))
    ...
    with ir.InsertionPoint(module.body):
        ...
        def mlir_matmul_multistage(a_host, b_host, c_host):
            ...
            with ir.InsertionPoint(launch_op.body.blocks[0]):
                ...
                for_op = scf.ForOp(c(0), c(num_stages), c(1))
                with ir.InsertionPoint(for_op.body):
                    iv = for_op.induction_variable
                ...
                with ir.InsertionPoint(for_op.body):
                    ...
                    b_tile_slice = memref.view(ir.MemRefType.get(b_tile_shape, f16, memory_space=smem_space),
                                               dynamic_smem, b_offset, [])
                    debug_print("[MainLoop] MMA a_offset={} b_offset={}", a_offset, b_offset, predicate=primaryThread)
                    da = nvgpu.WarpgroupGenerateDescriptorOp(a_wgmma_ty, a_tile_slice, a_tma_desc)
                    db = nvgpu.WarpgroupGenerateDescriptorOp(b_wgmma_ty, b_tile_slice, b_tma_desc)
                    ...
                    nextStage = arith.addi(iv, c(num_stages))
                    p1 = arith.cmpi(arith.CmpIPredicate.ult, nextStage, c(K // BLOCK_K))
                    ...
                    with ir.InsertionPoint(scf.IfOp(p).then_block):
                        ...
                        debug_print("[MainLoop] TMA Load a_offset={} b_offset={} b_offset2={} @ a=({},{}) b=({},{})",
                                a_offset,
                                b_offset,
                                b_offset2,
                                coord, dimX,
                        ...
                        debug_print("[MainLoop] mbarTMA[{}] arrive", stage, predicate=primaryThread)
                        nvgpu.mbarrier_arrive_expect_tx(mbarTMA, c(txcount), stage, predicate=primaryThread)
                        debug_print("[MainLoop] mbarTMA[{}] arrive [done]", stage, predicate=primaryThread)
                        scf.yield_([])
