...
def _is_contiguous_shape_slice(
  ...
  ss = sorted(list(zip(strides, shape)), key=lambda x: x[0], reverse=True)
...
def memref_unfold(ref: ir.Value, dim, factors) -> ir.Value:
  ...
  known_factor_prod = np.prod(list(f for f in factors if f is not None))
...
) -> tuple[tuple[ir.Value | int, ...], tuple[int, ...], tuple[bool, ...]]:
