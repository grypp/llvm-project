...
# from google3.experimental.users.apaszke.mosaic_gpu import _mosaic_gpu_ext
from dsl import dsl as mgpu
...
# PTXAS_PATH = resources.GetResourceFilename(
#     "google3/third_party/gpus/cuda_12_0/bin/ptxas"
# )
# NVDISASM_PATH = resources.GetResourceFilename(
#     "google3/third_party/gpus/cuda_12_0/bin/nvdisasm"
# )

# # Without CUDA_ROOT, MLIR is unable to find ptxas.
# CUDA_ROOT = os.path.dirname(os.path.dirname(PTXAS_PATH))
...
def _launch(
  ...
  if (profiler_spec is None) != (maybe_prof_buffer is None):
    raise ValueError
  grid_vals = [c(i, types.index()) for i in grid]
  block_vals = [c(i, types.index()) for i in block]
  ...
  launch_op = gpu.LaunchOp(
      ...
      dynamicSharedMemorySize=c(smem_bytes, types.i32()))
  launch_op.body.blocks.append(*([types.index()] * 12))  # Append an empty block
  smem = ir.Attribute.parse("#gpu.address_space<workgroup>")
  with ir.InsertionPoint(launch_op.body.blocks[0]):
    dynamic_smem = gpu.dynamic_shared_memory(
        ir.MemRefType.get(
            (ir.ShapedType.get_dynamic_size(),), types.i8(), memory_space=smem
        )
    )
    smem_refs = []
    ...
    for ref_ty, ref_bytes in zip(flat_refs, smem_ref_bytes):
      ...
      tile_smem = memref.view(
          ...
          dynamic_smem, c(dynamic_smem_offset, types.index()), [],
      )
